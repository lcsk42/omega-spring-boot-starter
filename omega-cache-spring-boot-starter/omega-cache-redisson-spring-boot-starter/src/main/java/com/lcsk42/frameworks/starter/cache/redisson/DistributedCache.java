...
public interface DistributedCache extends Cache {
   ...
   */
  <T> T safeGet(@NotBlank String key, Class<T> clazz, CacheLoader<T> cacheLoader, long timeout,
      TimeUnit timeUnit, RBloomFilter<String> bloomFilter, CacheGetFilter<String> cacheCheckFilter);
  ...
  /**
   * 安全存入缓存值并将键添加到布隆过滤器。 有助于防止缓存穿透、击穿和雪崩。 适用于对外暴露的接口。
   */
   ...
   */
  void safePut(@NotBlank String key, Object value, long timeout, TimeUnit timeUnit,
      RBloomFilter<String> bloomFilter);
